...
public class PDFExporter {
     ...
     */
    public static void exportToFile(String filePath, String title, List<String> headers, List<List<Object>> data) throws IOException {
    ...
    /**
     * Export receipt/invoice to PDF with beautiful formatting and Vietnamese font support
     * 
    ...
    public static boolean exportReceipt(Payment payment, List<PaymentItem> items, String fileName) {
        try {
            ...
            // Try to load system fonts (DejaVu Sans supports Vietnamese)
            String[] fontPaths = {
            ...
            for (String fontPath : boldFontPaths) {
                ...
                if (fontFile.exists()) {
                    try {
                        boldFont = PdfFontFactory.createFont(fontPath, "Identity-H", PdfFontFactory.EmbeddingStrategy.PREFER_EMBEDDED);
                        break;
                    } catch (Exception e) {
                        // Try next path
                    }
            ...
            for (PaymentItem item : items) {
                ...
                // Unit price
                itemsTable.addCell(createTableCell(String.format("%,d", item.getUnitPrice()), font, TextAlignment.RIGHT));
                // Total
                itemsTable.addCell(createTableCell(String.format("%,d", item.getTotalLine()), font, TextAlignment.RIGHT));
            }
