...
public class InventoryRepository {
    public List<Inventory> loadInventory(String path) throws IOException {
        InputStream input = getClass().getResourceAsStream(path);
        if (input == null) {
            throw new IllegalStateException("Not Found " + AppConfig.INVENTORY_TEST_DATA_TXT);
        }
    ...
    public void saveInventory(List<Inventory> inventoryList, String path) throws Exception {
        File file = AppConfig.getInventoryDataFile();
    ...
    public void AddInventory(List<Inventory> inventoryList, Inventory inventory) {
        if (inventory == null) {
    ...
    public boolean updateInventory(List<Inventory> inventoryList, int id, Inventory updatedInventory) {
    ...
    public boolean deleteInventory(List<Inventory> inventoryList, int id) {
        return inventoryList.removeIf(inventory -> inventory.getId() == id);
    }

    public List<Inventory> getLowStockItems(List<Inventory> list) {
    ...
    public Inventory searchBySku(List<Inventory> inventoryList, String sku) {
    ...
    public Inventory searchByName(List<Inventory> inventoryList, String name) {
        return inventoryList.stream()
    ...
    public synchronized int getQty(int productId) {
        File f = AppConfig.getInventoryDataFile();
        if (!f.exists())
            return 0;
        ...
        try (BufferedReader br = new BufferedReader(
            ...
            throw new RuntimeException("Error reading " + f.getAbsolutePath(), e);
        }
        return 0;
    }
